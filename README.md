...
**Redeem codes are live!** Head to **Extras → Redeem**, punch one in, and claim your reward. Each code works once per device.
...
1. Go to the [Releases](../../releases) page and download the latest `.apk` (e.g. `wuwalab-<version>.apk`).
...
| **Eisenhower Matrix Widget** | A second, separate home-screen widget — a read-only, at-a-glance view of your current Eisenhower Matrix quadrants. |
...
* See the [Announcements](#announcements) section above for currently active codes.
...
* A bubble that hovers over other apps, including the game itself — shown as the WuWaLab logo, so it's instantly recognizable among other floating bubbles.
...
WuWaLab ships with **two** independent home-screen widgets — add either or both:

* **Resource Widget** — Waveplates with a live "Full in Xh Ym" countdown, plus Crystals, Union Level and your Astrite total. Background artwork respects whatever widget skin you've equipped in the shop, falling back to the bundled art. Full landscape size shows the art on the right and darkens the left where the numbers sit; small and square sizes use an even scrim so every value stays readable. Tapping it opens a quick chooser — jump straight to the Dashboard, the Pull Planner, or your To-Do list, or set an alarm in your phone's own clock app. Refreshes every 30 minutes in the background, and instantly whenever you change a value in-app.
* **Eisenhower Matrix Widget** — a second, separate widget showing a read-only preview of your current Matrix quadrants (Do First, Schedule, Delegate, Eliminate) straight from your To-Do data, with a configurable header, item count, and tap target (Matrix, To-Do, or an in-between chooser).

#### Planning Tools

* **To-Do list** with reminders — creating a new task also quietly earns a small Argstrite reward, same as any other log.
* **Eisenhower Matrix** for sorting tasks by urgency and importance, with its own dedicated home-screen widget (see above).
...
No, and there are no plans for one. WuWaLab ships as a side-loaded Android `.apk` — the floating overlay and home-screen widgets it's built around aren't things iOS allows third-party apps to do.
...
<details>
 #wuwa #wuwalab #wuwa-lab #android #utility #wuwautility #wuwalabapp #wuwalabapk #wutheringwaves #wuwacompanion #astritetracker #pullplanner #wuwalabarglax wuwalab arglax wuwalab
</details>
